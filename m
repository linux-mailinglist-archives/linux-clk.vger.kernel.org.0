Return-Path: <linux-clk+bounces-976-lists+linux-clk=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-clk@lfdr.de
Delivered-To: lists+linux-clk@lfdr.de
Received: from ny.mirrors.kernel.org (ny.mirrors.kernel.org [147.75.199.223])
	by mail.lfdr.de (Postfix) with ESMTPS id 553E9806F07
	for <lists+linux-clk@lfdr.de>; Wed,  6 Dec 2023 12:51:20 +0100 (CET)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by ny.mirrors.kernel.org (Postfix) with ESMTPS id 76FAE1C20BB4
	for <lists+linux-clk@lfdr.de>; Wed,  6 Dec 2023 11:51:19 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id ED81A34575;
	Wed,  6 Dec 2023 11:51:16 +0000 (UTC)
...
--- a/MAINTAINERS
+++ b/MAINTAINERS
@@ -20763,6 +20763,14 @@ F:	Documentation/devicetree/bindings/phy/starfiv=
...
@@ -10,3 +10,4 @@ obj-$(CONFIG_CLK_STARFIVE_JH7110_AON)	+=3D clk-starfive=
-jh7110-aon.o
...
+obj-$(CONFIG_CLK_STARFIVE_JH8100_SYS)	+=3D jh8100/
...
diff --git a/drivers/clk/starfive/jh8100/clk-starfive-jh8100.h b/drivers/=
clk/starfive/jh8100/clk-starfive-jh8100.h
new file mode 100644
...
+
+#endif
diff --git a/drivers/clk/starfive/jh8100/clk-sys.c b/drivers/clk/starfive=
/jh8100/clk-sys.c
new file mode 100644
...
st =3D {
+	/* root */
+	STARFIVE__DIV(SYSCRG_CLK_VDEC_ROOT_PREOSC, "sys_clk_vdec_root_preosc", =
10,
+		      SYSCRG_CLK_PLL7_OUT),
+	STARFIVE__MUX(SYSCRG_CLK_VDEC_ROOT, "sys_clk_vdec_root", 2,
+		      SYSCRG_CLK_OSC, SYSCRG_CLK_VDEC_ROOT_PREOSC),
+	STARFIVE__DIV(SYSCRG_CLK_VENC_ROOT_PREOSC, "sys_clk_venc_root_preosc", =
10,
+		      SYSCRG_CLK_PLL7_OUT),
...
+	STARFIVE__DIV(SYSCRG_CLK_VOUT_ROOT0_PREOSC, "sys_clk_vout_root0_preosc"=
, 127,
+		      SYSCRG_CLK_PLL1_OUT),
+	STARFIVE__MUX(SYSCRG_CLK_VOUT_ROOT0, "sys_clk_vout_root0", 2,
+		      SYSCRG_CLK_OSC, SYSCRG_CLK_VOUT_ROOT0_PREOSC),
+	STARFIVE__DIV(SYSCRG_CLK_VOUT_ROOT1_PREOSC, "sys_clk_vout_root1_preosc"=
, 127,
+		      SYSCRG_CLK_PLL6_OUT),
...
+		      SYSCRG_CLK_PLL3_OUT),
+	STARFIVE__DIV(SYSCRG_CLK_PERH_ROOT_PREOSC, "sys_clk_perh_root_preosc", =
4,
+		      SYSCRG_CLK_PLL0_OUT),
...
+	STARFIVE__MUX(SYSCRG_CLK_AXI_400_APBOOTRAM, "sys_clk_axi_400_apbootram"=
, 2,
+		      SYSCRG_CLK_OSC, SYSCRG_CLK_AXI_400_PREOSC),
...
+	STARFIVE_GATE(SYSCRG_CLK_IMG_GPU_RTC_TOGGLE, "sys_clk_img_gpu_rtc_toggl=
e", 0,
+		      SYSCRG_CLK_OSC),
+	STARFIVE_GATE(SYSCRG_CLK_IMG_GPU_TIMER_USC, "sys_clk_img_gpu_timer_usc"=
, 0,
+		      SYSCRG_CLK_OSC),
+	/* hifi4 (se) */
+	STARFIVE__DIV(SYSCRG_CLK_HIFI4_CORE_PREOSC, "sys_clk_hifi4_core_preosc"=
, 15,
+		      SYSCRG_CLK_PLL0_OUT),
...
+	STARFIVE_GATE(SYSCRG_CLK_FLEXNOC_PCIEX1AMST, "sys_clk_flexnoc_pciex1ams=
t",
+		      CLK_IS_CRITICAL, SYSCRG_CLK_AXI_500_PCIEX1A),
+	STARFIVE_GATE(SYSCRG_CLK_FLEXNOC_PCIEX1ASLV, "sys_clk_flexnoc_pciex1asl=
v",
+		      CLK_IS_CRITICAL, SYSCRG_CLK_AXI_500_PCIEX1A),
+	STARFIVE_GATE(SYSCRG_CLK_FLEXNOC_PCIEX1BMST, "sys_clk_flexnoc_pciex1bms=
t",
+		      CLK_IS_CRITICAL, SYSCRG_CLK_AXI_500_PCIEX1B),
+	STARFIVE_GATE(SYSCRG_CLK_FLEXNOC_PCIEX1BSLV, "sys_clk_flexnoc_pciex1bsl=
v",
+		      CLK_IS_CRITICAL, SYSCRG_CLK_AXI_500_PCIEX1B),
...
,
+		      CLK_IS_CRITICAL, SYSCRG_CLK_AXI_500_PCIEX8),
+	STARFIVE_GATE(SYSCRG_CLK_FLEXNOC_GMACSYSSLV, "sys_clk_flexnoc_gmacsyssl=
v",
+		      CLK_IS_CRITICAL, SYSCRG_CLK_AXI_200_GMAC),
+	/* gmac1 (se) */
+	STARFIVE__DIV(SYSCRG_CLK_GMAC_SRC, "sys_clk_gmac_src", 7,
+		      SYSCRG_CLK_PLL0_OUT),
+	STARFIVE__DIV(SYSCRG_CLK_GMAC1_GTXCLK_TOP, "sys_clk_gmac1_gtxclk_top", =
400,
+		      SYSCRG_CLK_PLL0_OUT),
+	STARFIVE__DIV(SYSCRG_CLK_GMAC1_PTP, "sys_clk_gmac1_ptp", 31,
+		      SYSCRG_CLK_GMAC_SRC),
+	/* hd audio */
+	STARFIVE_GATE(SYSCRG_CLK_HD_AUDIO_SYSTEM_CLOCK, "sys_clk_hd_audio_syste=
m_clock",
+		      0, SYSCRG_CLK_APB_BUS_PER7),
+	STARFIVE_GATE(SYSCRG_CLK_HD_AUDIO_CLOCK_48, "sys_clk_hd_audio_clock_48"=
,
+		      0, SYSCRG_CLK_HD_AUDIO_48M),
+	STARFIVE_GATE(SYSCRG_CLK_HD_AUDIO_BCLK_POST_OCC_IN, "sys_clk_hd_audio_b=
clk_post_occ_in",
+		      0, SYSCRG_CLK_HD_AUDIO_48M),
+	/* nne_vip (se) */
...
+	STARFIVE_GMUX(SYSCRG_CLK_NNE_VIP_CLKCORE, "sys_clk_nne_vip_clkcore", 0,=
 2,
+		      SYSCRG_CLK_PLL2_OUT,
+		      SYSCRG_CLK_PLL0_OUT),
+	/* icg_en */
...
+
+	/* 24MHz -> 2000.0MHz */
+	priv->pll[0] =3D devm_clk_hw_register_fixed_factor(priv->dev, "clk_pll0=
_out",
+							 "clk_osc", 0, 250, 3);
...
+
+	/* 24MHz -> 1782.0MHz */
+	priv->pll[1] =3D devm_clk_hw_register_fixed_factor(priv->dev, "clk_pll1=
_out",
+							 "clk_osc", 0, 445, 6);
...
+
+	/* 24MHz -> 1843.2MHz */
+	priv->pll[2] =3D devm_clk_hw_register_fixed_factor(priv->dev, "clk_pll2=
_out",
+							 "clk_osc", 0, 767, 10);
...
+
+	/* 24MHz -> 1866MHz */
+	priv->pll[3] =3D devm_clk_hw_register_fixed_factor(priv->dev, "clk_pll3=
_out",
+							 "clk_osc", 0, 4665, 60);
...
+
+	/* 24MHz -> 2000MHz */
+	priv->pll[4] =3D devm_clk_hw_register_fixed_factor(priv->dev, "clk_pll4=
_out",
+							 "clk_osc", 0, 250, 3);
...
+
+	/* 24MHz -> 1782MHz */
+	priv->pll[5] =3D devm_clk_hw_register_fixed_factor(priv->dev, "clk_pll6=
_out",
+							 "clk_osc", 0, 445, 6);
...
+
+	/* 24MHz -> 2400MHz */
+	priv->pll[6] =3D devm_clk_hw_register_fixed_factor(priv->dev, "clk_pll7=
_out",
+							 "clk_osc", 0, 100, 1);
+	if (IS_ERR(priv->pll[6]))
+		return PTR_ERR(priv->pll[6]);
+
+	for (idx =3D 0; idx < SYSCRG_CLK_END; idx++) {
...
+			else if (pidx =3D=3D SYSCRG_CLK_OSC)
+				parents[i].fw_name =3D "clk_osc";
+			else if (pidx =3D=3D SYSCRG_CLK_MCLK_EXT)
+				parents[i].fw_name =3D "clk_mclk_ext";
+			else if (pidx =3D=3D SYSCRG_CLK_PLL1_OUT && !priv->pll[1])
+				parents[i].fw_name =3D "clk_pll1_out";
+			else if (pidx =3D=3D SYSCRG_CLK_PLL2_OUT && !priv->pll[2])
+				parents[i].fw_name =3D "clk_pll2_out";
+			else if (pidx =3D=3D SYSCRG_CLK_PLL6_OUT && !priv->pll[5])
+				parents[i].fw_name =3D "clk_pll6_out";
+			else
+				parents[i].hw =3D priv->pll[pidx - SYSCRG_CLK_PLL0_OUT];
+		}
+
+		clk->hw.init =3D &init;
+		clk->idx =3D idx;
